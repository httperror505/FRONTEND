...
function SearchResult({ result }) {
  ...
  const file_name = result.file_name; // Extract documentID from result
  ...
  useEffect(() => {
    const fetchPDF = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`https://node-js-backend-almario1.onrender.com/pdf/${file_name}`, {
  ...
  }, [file_name, showModal, selectedFormat]);
